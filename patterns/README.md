...
| `tooltip.ts` | Global `[data-tip]` hover tooltip: one floating div, delegated listeners, viewport-edge flipping | au-flights `src/tooltip.ts` | Copy to `src/components/tooltip.ts`, call `initTooltip()` once at boot, put `data-tip="…"` on every data mark (SVG or HTML). **The CSS is not optional and is not in this file** — see the note below. Native SVG `<title>` is NOT a tooltip — use `aria-label` for a11y instead. |
...
## `tooltip.ts` needs two CSS rules that live outside it

`initTooltip()` shows and hides purely by toggling a `visible` class. If the
stylesheet defines `.hover-tip` but **not** `.hover-tip.visible`, the tooltip is
painted on the first hover and **never hidden again** — it simply follows the
pointer around the page, covering whatever is under it. Nothing errors, the class
is toggled correctly the whole time, and no test that checks the handler will see
it. Ship both:

```css
.hover-tip { /* fixed, high z-index, pointer-events: none, … */
  opacity: 0; transition: opacity 110ms ease; }
.hover-tip.visible { opacity: 1; }
```

Assert it in the test suite alongside the `[hidden]` guard — a two-line source
check on `styles.css` costs nothing and catches a defect that is invisible in
jsdom.
